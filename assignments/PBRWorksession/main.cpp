	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	{
		...
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 10; j++)
			{
				...
				shader.use();
				shader.setFloat("_Material.metalic", currMat.metalic);
				shader.setFloat("_Material.roughness", currMat.roughness);
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		render(litShader, lightVisShader, postProcessShader, pbrShader, suzanne, suzanneTransform, Rock_Color, rockNormal, deltaTime);
